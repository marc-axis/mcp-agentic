...
export function userCreatePage(teams, prefillTeamId = "", error = "", csrfToken = "") {
  const body = `
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-6 max-w-md">
      ${error ? `<div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">${escHtml(error)}</div>` : ""}
      <form method="POST" action="/admin/users/create" class="flex flex-col gap-4">
        ...
        <div>
          ...
          <select id="team_id" name="team_id"
            ...
            ${teams.map(t => `<option value="${escHtml(t.id)}" ${t.id === prefillTeamId ? "selected" : ""}>${escHtml(t.name)}</option>`).join("")}
          </select>
        </div>
        <button type="submit"
  ...
  const body = `
    <div class="max-w-md">
      <div class="bg-green-50 border border-green-200 rounded-xl p-5 mb-6 flex items-start gap-3">
        ...
        <div>
          <div class="text-sm font-semibold text-green-800">User created: ${escHtml(clientName)}</div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-5">
        ...
        <div class="flex gap-2">
          <code id="api-key" class="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm mono break-all">${escHtml(apiKey)}</code>
